...
NUMER_RACHUNKU Bank::OtworzZdebetem(string klient, string haslo, double debet)
{
	...
	Konto* konto = new Konto(klient, haslo, accountNumber, debet);
...
NUMER_RACHUNKU Bank::Otworz(string klient, string haslo)
{
	...
	Konto* konto = new Konto(klient, haslo, accountNumber, 0);// doda³em zero
...
bool Bank::DodajDebet(NUMER_RACHUNKU numer, int id, double debet) {

	return this->Znajdz(numer)->DodajDebet(id, debet);
}
